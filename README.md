...
- El directorio **src** ahora cuenta con un nuevo directorio llamado **notificaciones**, el cual representa un servicio de mensajería que recibe eventos de integración propagados del sistema de propiedades, por medio de un broker de eventos.
...
- Nuestro proyecto de propiedades ha cambiado de forma considerable. Los siguientes son los cambios relevantes en cada módulo:
    - **api**: En este módulo se modificó el API de `clientes.py` el cual cuenta con dos nuevos endpoints: `/cliente-commando` y `/cliente-query`, los cuales por detrás de escenas usan un patrón CQRS como la base de su comunicación.
    - **modulos/../aplicacion**: Este módulo ahora considera los sub-módulos: `queries` y `comandos`. En dichos directorios pdrá ver como se desacopló las diferentes operaciones lectura y escritura. Vea en el módulo `clientes` los archivos `obtener_cliente.py` y `crear_cliente.py` para ver como se logra dicho desacoplamiento.
    - **modulos/../aplicacion/handlers.py**: Estos son los handlers de aplicación que se encargan de oir y reaccionar a eventos. Si consulta el módulo de clientes podra ver que tenemos handlers para oir y reaccionar a los eventos de dominio para poder continuar con una transacción. En el modulo de clientes encontramos handlers para eventos de integración los cuales pueden ser disparados desde la capa de infraestructura, la cual está consumiendo eventos y comandos del broker de eventos.
...
```bash
docker build . -f adaptador.Dockerfile -t propiedades/adaptador
```
...
```bash
docker run -p 50051:50051 propiedades/adaptador
```
...
```bash
docker build . -f notificacion.Dockerfile -t propiedades/notificacion
```
...
```bash
docker run propiedades/notificacion
```
...
```bash
docker build . -f ui.Dockerfile -t propiedades/ui
```
...
```bash
docker-compose --profile <pulsar|propiedades|ui|notificacion> up
```
